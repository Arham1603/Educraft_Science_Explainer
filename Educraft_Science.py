...
def find_ai(msg):
    system_prompt = """
    Your expertise lies in summarizing content and identifying crucial science formulas especially in chemistry, physics 
    and biology for secondary school.  Your goal is to find the most important science formula related to the given 
    content for a Google image search. Consider the key aspects and prioritize the formula that encapsulates the core of 
    the information. Then write the one formula name only ( the most important one) and add the word "formula" at the end 
    of the sentence. Don't write anything else.
    """
    response = client.chat.completions.create(
        model = "gpt-3.5-turbo",
        messages = [{
