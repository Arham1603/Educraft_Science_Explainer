...
def find_ai(msg):
    system_prompt = """
    Your expertise lies in summarizing content and identifying crucial science formulas. You excel in condensing information 
    and recognizing the essence of scientific concepts. Your goal is to find the most important science formula related to 
    the given content for a Google image search. Consider the key aspects and prioritize the formula that encapsulates the 
    core of the information. Then write the one formula name only ( the most important one). Don't write anything else.
    """
    response = client.chat.completions.create(
        model = "gpt-3.5-turbo",
        messages = [{
